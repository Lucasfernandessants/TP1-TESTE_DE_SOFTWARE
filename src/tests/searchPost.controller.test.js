...
describe('searchPost Controller', () => {
    test('deve buscar posts quando query for fornecida', () => {
        ...
        expect(mockPrepare).toHaveBeenCalledWith(expect.stringContaining('WHERE title LIKE ?'));
    ...
    test('deve retornar todos os posts quando a query estiver vazia', () => {
        ...
        const mockPrepare = jest.fn().mockReturnValue({ get: mockGet, all: mockAll });
        const mockDb = { prepare: mockPrepare };
        ...
        expect(mockPrepare).toHaveBeenCalledWith(expect.not.stringContaining('WHERE title LIKE ?'));
